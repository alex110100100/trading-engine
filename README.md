...
| `GET` | `/order/{id}` | Latest status for **submitted** orders (and after cancel / replay): **in-memory** when present, else **from `order_states`** when JPA is enabled. Returns `{ "orderId", "status" }` or **404** if unknown. |
...
- **Order status:** Each **submit** and **cancel** updates **`order_states`** (when JPA is enabled); replay also writes status for restored open orders. **`GET /order/{id}`** checks memory first, then the database.
